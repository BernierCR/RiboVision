...
Clazz.load (["J.api.SymmetryInterface"], "J.symmetry.Symmetry", ["java.lang.Float", "java.util.Hashtable", "JU.BS", "$.M3", "$.P3", "$.SB", "J.modelset.Atom", "J.symmetry.PointGroup", "$.SpaceGroup", "$.SymmetryInfo", "$.SymmetryOperation", "$.UnitCell", "J.util.Escape", "$.Logger", "$.SimpleUnitCell"], function () {
...
$_V(c$, "addOp", 
function (code, rs, vs, sigma, jtoi) {
...
this.spaceGroup.operations[i].setSigma (code, sigma, jtoi);
System.out.println (this.spaceGroup.operations[i]);
return s;
}, "~S,JU.Matrix,JU.Matrix,JU.Matrix,JU.M3");
...
this.unitCell.matrixCartesianToFractional.getRotationScale (m);
} else if (type.equals ("toCartesian")) {
this.unitCell.matrixFractionalToCartesian.getRotationScale (m);
}return m;
}, "~S");
});
